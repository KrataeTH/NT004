...
if (!empty($events['events'])) {
    foreach ($events['events'] as $event) {
        if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
            ...
            // ตรวจสอบว่าข้อความเป็น code_services (ตัวเลขและตัวอักษร)
            elseif (preg_match('/^[a-zA-Z0-9]+$/', $userMessage)) {
                searchCustomerByServiceCode($replyToken, $userMessage, $conn, $access_token);
            }
...
// ฟังก์ชันค้นหาลูกค้าด้วย code_services
function searchCustomerByServiceCode($replyToken, $serviceCode, $conn, $accessToken) {
    // คำสั่ง SQL สำหรับค้นหาลูกค้าจาก code_services
    $sql = "SELECT c.id_customer, c.name_customer, sc.code_services, 
                   COALESCE(SUM(o.all_price), 0) as total_revenue, 
                   COUNT(DISTINCT sc.id_service) as service_count,
                   GROUP_CONCAT(DISTINCT sc.code_services SEPARATOR ', ') as code_services
            FROM customers c
    ...
    if ($result->num_rows > 0) {
        ...
        $message .= "📝 เลขหมาย: " . $customer['code_services'] . "\n";
        ...
        sendReply($replyToken, "❌ ไม่พบข้อมูลลูกค้าสำหรับเลขหมาย $serviceCode", $accessToken);
    }
}
    ...
    $menu = [
        ...
        "template" => [
            ...
            "actions" => [
                ...
                ["type" => "message", "label" => "🔍 ค้นหาด้วยเลขหมาย", "text" => "ค้นหาด้วยเลขหมาย"]
            ]
        ]
    ];
